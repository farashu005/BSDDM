...
//
// void leap_frog_stop_param(const vec &tau, const vec &tau_s, const double sigma,const vec &delta_prime,
//                                const vec &SSD,const double DEL,const double DEL_s,
//                                const vec &b_l, const vec &b_r,
//                                const vec &nu_l, const vec &nu_r, const vec &nu_l_p, const vec &nu_r_p,
//                                const vec &nu_l_squared, const vec &nu_r_squared,
...
//                                const uvec &Ind_S_LCR, const uvec &Ind_S_LIR, const uvec &Ind_S_RCR,const uvec &Ind_S_RIR,
//                                const uvec &Ind_I_L, const uvec &Ind_I_R,
//                                vec &stop_param,const vec &penal_param,const vec &prob_param,
//                                const double mu_b_stop, const double sigma_b_stop, const double mu_nu_stop,const double sigma_nu_stop,
//
//                                const vec &prob_hyp,
//                                const double T_Go,const double k,const double T_Total,
//
//                                vec &v_old_stop,const double L, const double delta,
//                                mat &p_stop,
//
//
//                                const vec &lower_bound, const double upper_bound, const bool lt = 1) {
...
//
//
//   void leap_frog_prob_param(const vec &tau, const vec &tau_s, const double sigma,const vec &delta_prime,
//                                  const vec &SSD,const double DEL,const double DEL_s,
//                                  const vec &b_l, const vec &b_r,
//                                  const vec &nu_l, const vec &nu_r, const vec &nu_l_p, const vec &nu_r_p,
//                                  const vec &nu_l_squared, const vec &nu_r_squared,
...
//                                  const uvec &Ind_S_LCR, const uvec &Ind_S_LIR, const uvec &Ind_S_RCR,const uvec &Ind_S_RIR,
//                                  const uvec &Ind_I_L, const uvec &Ind_I_R,
//                                  const vec &stop_param,const vec &penal_param,vec &prob_param,
//                                  const double mu_b_stop, const double sigma_b_stop, const double mu_nu_stop,const double sigma_nu_stop,
//
//                                  const vec &prob_hyp,
//                                  const double T_Go,const double k,const double T_Total,
//
//                                  vec &v_old_prob,const double L, const double delta,
//                                  mat &p_prob,
//
//
//                                  const vec &lower_bound, const double upper_bound, const bool lt = 1) {
  ...
  for (unsigned i = 0; i < nhmc; ++i){
    ...
    if(update_stop_prob==1){
      ...
      update_stop_prob_param(tau_prime,tau_s,sigma,SSD,DEL,DEL_s,Ind_GF,
                             ...
                             lower_bound,upper_bound,lt);
      //
      //       update_stop_param(tau_prime,tau_s,sigma,SSD,DEL,DEL_s,Ind_GF,
      //                              delta_param,gama,beta,penal_param,stop_param,prob_param,
      //                              b_l,b_r,nu_l,nu_r,nu_l_p,nu_r_p,
      //                              nu_l_squared,nu_r_squared,
      //                              Ind_L,Ind_R,
      ...
      //                              prob_hyp,T_Go,k,T_Total,
      //
      //                              range_stop, L,leapmax,acceptance_stop,nparall,
      //                              lower_bound,upper_bound,lt);
    ...
    // if(update_prob==1){
    //
    // update_prob_param(tau_prime,tau_s,sigma,SSD,DEL,DEL_s,Ind_GF,
    //                   delta_param,gama,beta,penal_param,stop_param,prob_param,
    //                   b_l,b_r,nu_l,nu_r,nu_l_p,nu_r_p,
    //                   nu_l_squared,nu_r_squared,
    //                   Ind_L,Ind_R,
    ...
    //                   prob_hyp,T_Go,k,T_Total,
    //
    //                   range_prob, L,leapmax,acceptance_prob,nparall,
    //                   lower_bound,upper_bound,lt);
    //
    // }
    ...
    if (i % thin == 0) {
      ...
      // << " Med_accept_stop = " << median(acceptance_stop) / ((double)i)
      // << " Med_accept_prob = " << median(acceptance_prob) / ((double)i)
    ...
    if ((i + 1) % checkpoint_every == 0) {
      ...
      // out_partial["Accept_Stop"]=acceptance_stop/denom;
      // out_partial["Accept_Prob"]=acceptance_prob/denom;

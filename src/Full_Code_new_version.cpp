...
mat integrate_penalty_Right_Stim(const field<vec> &integral_likelihood,const double sigma,
...
void leap_frog_rand_effect(const mat &Phi,const vec &b_l_main,const vec &b_r_main,
                           ...
                           mat &v_old_g_I,mat &v_old_b_I, const unsigned L, const double delta,
                           mat &p_g_I,mat &p_b_I,
...
void update_rand_effect(const mat &Phi,
                        const vec &b_l_main,const vec &b_r_main,
                        const vec &nu_l_main,const vec &nu_r_main,
                        const vec &S_gama_l,const vec &S_gama_r,
                        const vec &S_beta_l,const vec &S_beta_r,
                        const vec &rand_param_g_l,const vec &rand_param_g_r,
                        const vec &rand_param_b_l,const vec &rand_param_b_r,
                        const vec &tau,const vec &tau_s,const double sigma,const vec &SSD,const double DEL,const double &DEL_s,
                        const vec &Ind_GF,const vec &delta_prime,
                        const vec &penal_param,const vec &stop_param,const vec &prob_param,
                        ...
                        const double T_Go,const double k,
                        
                        vec &b_l,vec &nu_l,vec &b_r,vec &nu_r,
                        
                        vec &nu_l_p,vec &nu_r_p,vec &nu_l_s,vec &nu_r_s,
                        
                        vec &nu_l_squared,vec &nu_r_squared,
                        
                        
                        vec &weight_LCR, vec &weight_LIR,vec &weight_RCR, vec &weight_RIR,
                        vec &weight_LCR_S, vec &weight_LIR_S,vec &weight_RCR_S, vec &weight_RIR_S,
                        
                        vec &weight_LCR_G_FS, vec &weight_LIR_G_FS,vec &weight_RCR_G_FS, vec &weight_RIR_G_FS,
                        
                        
                        field<vec> &lk_FS,field<vec> &likelihood_integral,
                        
                      
                  
                        const vec &range,const unsigned L,const int leapmax,
                        const unsigned nparall,
                        const vec &lower_bound,const double upper_bound,
                        
                        bool &accpt_rand_samp,
  ...
  // Rcpp::Rcout<<"ABA"<< endl;
    ...
    mat p_g_I(size(gama_I),fill::randn);
    
    mat p_b_I(size(beta_I),fill::randn);
    
    
    double U_old = -log_lhood(Inhib_likelihood,
                              sigma, tau, tau_s, SSD,Ind_GF,
    ...
    double prior_main_g_I=mvn_prior2(rand_param_g_l(0),rand_param_g_r(0),gama_I,S_gama_l,S_gama_r);
    
    double prior_main_b_I=mvn_prior2(rand_param_b_l(0),rand_param_b_r(0),
                                     beta_I,S_beta_l,S_beta_r);
    ...
    mat v_old_g_I(size(gama_I), fill::zeros);
    
    v_old_g_I =- grad_gama(Phi, tau,tau_s, sigma, SSD,DEL,DEL_s,
                           nu_l, nu_r, b_l, b_r,
                           nu_l_p, nu_r_p, nu_l_s, nu_r_s,
                           weight_LCR,weight_LIR,weight_RCR,weight_RIR,
                           weight_LCR_G_FS,weight_LIR_G_FS,weight_RCR_G_FS,weight_RIR_G_FS,
    ...
    mat gama_I_prior=grad_Ind_prior(gama_I,S_gama_l,S_gama_r,rand_param_g_l(0),rand_param_g_r(0));
    
    v_old_g_I -=gama_I_prior;
    
    
    mat gama_I_new=gama_I;
    ...
    mat v_old_b_I(size(beta_I), fill::zeros);
    
    
    v_old_b_I =-grad_beta(Phi,tau,tau_s,sigma,SSD,DEL,DEL_s,
                          nu_l,nu_l_p,nu_r,nu_r_p,nu_l_s,nu_r_s,
                          b_l,b_r,nu_l_squared,nu_r_squared,
                          penal_param,Ind_L,Ind_R,Ind_LCR,Ind_LIR,Ind_RIR,Ind_RCR,
                          Ind_S_LCR,Ind_S_LIR,Ind_S_RIR,Ind_S_RCR,
    ...
    leap_frog_rand_effect(Phi,b_l_main,b_r_main,
                          nu_l_main,nu_r_main,
                          S_gama_l,S_gama_r,
                          ...
                          tau,tau_s,sigma,SSD,DEL,DEL_s,
                          delta_prime,penal_param,stop_param,prob_param,
                          Ind_L,Ind_R,Ind_LCR,Ind_LIR,Ind_RCR,Ind_RIR,
                          Ind_S_LCR,Ind_S_LIR,Ind_S_RCR,Ind_S_RIR,
                          Ind_I_L,Ind_I_R,gama_I_new,beta_I_new,
    ...
    vec b_l_new = b_l_main+ (Phi * gama_I_new.col(0));
    vec b_r_new = b_r_main+ (Phi * gama_I_new.col(1));
    
    
    vec nu_l_new = nu_l_main+ (Phi * beta_I_new.col(0));
    vec nu_l_squared_new = square(nu_l_new);
    
    vec nu_r_new = nu_r_main+ (Phi * beta_I_new.col(1));
    vec nu_r_squared_new = square(nu_r_new);
    
    
    vec nu_l_p_new = h(nu_l_new, Ind_R ) - penalty_fun(h(nu_r_new, Ind_R ),h(nu_r_squared_new, Ind_R ),penal_param);
    vec nu_r_p_new = h(nu_r_new, Ind_L ) - penalty_fun(h(nu_l_new, Ind_L ),h(nu_l_squared_new, Ind_L ),penal_param);
    
    
    vec nu_l_s_new=nu_l_new-penalty_fun_s(stop_param(1),penal_param);
    vec nu_r_s_new=nu_r_new-penalty_fun_s(stop_param(1),penal_param);
    
    
    
    field<vec> likelihood_integral_new=integral_likelihood2(sigma,SSD,DEL,DEL_s,stop_param,
                                                           penal_param,prob_param,
    ...
    field<vec> lk_FS_new= update_lk_FS4(tau,tau_s,sigma,SSD,
    ...
    accpt_rand_samp=log(randu()) < -(H_new - H_old);
    ...
    if (accpt_rand_samp) {
      ...
      nu_l = nu_l_new;
      nu_l_squared = nu_l_squared_new;
      
      nu_r = nu_r_new;
      nu_r_squared= nu_r_squared_new;
      ...
      likelihood_integral = likelihood_integral_new;
      
      lk_FS=lk_FS_new;
      
      
      
      weight_LCR=weight(sigma,g(tau, Ind_L, Ind_LCR ),h(nu_r_p,Ind_LCR ),g(b_r, Ind_L, Ind_LCR ));
      
      weight_LIR=weight(sigma,g(tau, Ind_L, Ind_LIR ),g(nu_l, Ind_L, Ind_LIR ),g(b_l, Ind_L, Ind_LIR ));
      
      weight_RCR=weight(sigma,g(tau, Ind_R, Ind_RCR ),h(nu_l_p, Ind_RCR ),g(b_l, Ind_R, Ind_RCR));
      
      weight_RIR=weight(sigma,g(tau, Ind_R, Ind_RIR ),g(nu_r, Ind_R, Ind_RIR ),g(b_r, Ind_R, Ind_RIR ));
      
      
      weight_LCR_G_FS=weight(sigma,g(tau, Ind_L, Ind_S_LCR ),h(nu_r_p, Ind_S_LCR ),g(b_r, Ind_L, Ind_S_LCR ));
      
      weight_LIR_G_FS=weight(sigma,g(tau, Ind_L, Ind_S_LIR ),g(nu_l, Ind_L, Ind_S_LIR ),g(b_l, Ind_L, Ind_S_LIR ));
      
      weight_RCR_G_FS=weight(sigma,g(tau, Ind_R, Ind_S_RCR ),h(nu_l_p, Ind_S_RCR ),g(b_l, Ind_R, Ind_S_RCR ));
      
      weight_RIR_G_FS=weight(sigma,g(tau, Ind_R, Ind_S_RIR ),g(nu_r, Ind_R, Ind_S_RIR ),g(b_r, Ind_R, Ind_S_RIR ));
      
      
      weight_LCR_S=weight_s(sigma,g(tau, Ind_L, Ind_S_LCR ),g(tau_s, Ind_L, Ind_S_LCR ),g(SSD, Ind_L, Ind_S_LCR ),
                                g(b_r, Ind_L, Ind_S_LCR ),h(nu_r_p, Ind_S_LCR ),
                                g(nu_r_s, Ind_L, Ind_S_LCR ),DEL,DEL_s,lt);
      
      weight_LIR_S=weight_s(sigma,g(tau, Ind_L, Ind_S_LIR ),g(tau_s, Ind_L, Ind_S_LIR ),g(SSD, Ind_L, Ind_S_LIR ),
                                g(b_l, Ind_L, Ind_S_LIR ),g(nu_l,Ind_L, Ind_S_LIR ),
                                g(nu_l_s, Ind_L, Ind_S_LIR ),DEL,DEL_s,lt);
      
      weight_RCR_S=weight_s(sigma,g(tau, Ind_R, Ind_S_RCR ),g(tau_s, Ind_R, Ind_S_RCR ),g(SSD, Ind_R, Ind_S_RCR ),
                                g(b_l, Ind_R, Ind_S_RCR ),h(nu_l_p, Ind_S_RCR ),
                                g(nu_l_s,Ind_R, Ind_S_RCR ),DEL,DEL_s,lt);
      
      weight_RIR_S=weight_s(sigma, g(tau, Ind_R, Ind_S_RIR ),g(tau_s, Ind_R, Ind_S_RIR ),g(SSD, Ind_R, Ind_S_RIR ),
                                g(b_r, Ind_R, Ind_S_RIR ),g(nu_r, Ind_R, Ind_S_RIR ),
                                g(nu_r_s,Ind_R, Ind_S_RIR ),DEL,DEL_s,lt);
...
void leap_frog_penal_param(const vec &tau,const vec &tau_s,const double sigma,
  ...
  for (unsigned j = 0; j < L; ++j){
    ...
    mat Penal_RS =integrate_penalty_Right_Stim(likelihood_integral,sigma,SSD,DEL,DEL_s,
                                             penal_param,stop_param,prob_param,
                                             b_l,b_r,nu_l,nu_r,
                                             nu_l_s,nu_r_s,
                                             nu_l_p,nu_r_squared,
...
void update_penal_param(const vec &tau,const vec &tau_s,const double sigma,const vec &SSD,
                        const double DEL,const double DEL_s,const vec &Ind_GF,
                        const vec &delta_prime,const mat &gama,const mat &beta,vec &penal_param,const vec &stop_param,
                        const vec &b_l,const vec &b_r,
                        const vec &nu_l,const vec &nu_r,
                        const vec &nu_l_squared,const vec &nu_r_squared,
  ...
  // Rcpp::Rcout<<"update"<< endl;
    ...
    vec p(size(penal_param),fill::randn);
    ...
    double prior_main=log_normal_prior(penal_param,mu_lambda_prime,sigma_lambda_prime,
                                       mu_alpha_prime,sigma_alpha_prime);
    ...
    vec v_old =- grad_penal_param(tau,tau_s,sigma,SSD,DEL,DEL_s,
                                  b_l,b_r,nu_l,nu_r,nu_r_p,nu_l_p,
                                  nu_l_squared,nu_r_squared,nu_l_s,nu_r_s,
                                  Ind_L,Ind_R,Ind_LCR,Ind_LIR,Ind_RIR,Ind_RCR,
                                  Ind_S_LCR,Ind_S_LIR,Ind_S_RIR,Ind_S_RCR,
    ...
    vec penal_param_new=penal_param;
    vec v_new=v_old;
    ...
    leap_frog_penal_param(tau,tau_s,sigma,delta_prime,SSD,DEL,DEL_s,
    ...
    vec nu_l_p_i_new= h(nu_l, Ind_R ) - penalty_fun(h(nu_r, Ind_R ),h(nu_r_squared, Ind_R ),penal_param_new);
    vec nu_r_p_i_new= h(nu_r, Ind_L ) - penalty_fun(h(nu_l, Ind_L ),h(nu_l_squared, Ind_L ),penal_param_new);



    vec nu_l_s_i_new=nu_l-penalty_fun_s(stop_param(1),penal_param_new);
    vec nu_r_s_i_new=nu_r-penalty_fun_s(stop_param(1),penal_param_new);




    field<vec> likelihood_integral_new=integral_likelihood2(sigma,SSD,DEL,DEL_s,stop_param,
                                                           penal_param_new,prob_param,
                                                           b_l,nu_l,nu_l_s_i_new,
                                                           b_r,nu_r_p_i_new,nu_r_s_i_new,
                                                           nu_r,nu_l_p_i_new,
                                                           Ind_L,Ind_R,Ind_I_L,Ind_I_R,
                                                           lower_bound,upper_bound,lt);
    ...
    field<vec> lk_FS_new= update_lk_FS4(tau,tau_s,sigma,SSD,
                                        b_l,b_r,nu_l,nu_r,nu_r_p_i_new,nu_l_p_i_new,
    ...
    double U_new=-  log_lhood(Inhib_likelihood_new,
                              sigma, tau, tau_s, SSD,Ind_GF,
                              b_l, b_r, nu_l, nu_r, nu_l_p_i_new,nu_r_p_i_new,
                              nu_l_s_i_new,nu_r_s_i_new,
                              Ind_L, Ind_R, Ind_LCR, Ind_LIR, Ind_RIR, Ind_RCR,
                              Ind_S_LCR, Ind_S_LIR, Ind_S_RIR, Ind_S_RCR,
                              lk_LCR_FS_new, lk_LIR_FS_new,lk_RCR_FS_new,lk_RIR_FS_new,
    ...
    accpt_p_samp= log(randu())< -(H_new-H_old);


    if(accpt_p_samp){

    penal_param=penal_param_new;
...
void update_delta_param(const vec &tau,const vec &tau_stop,vec &tau_prime,vec &tau_s,
                        const double sigma,const vec &SSD,const double SSD_min,const double U,
                        
                        double &DEL, double &DEL_s,
                        
                        const vec &Ind_GF,
                        vec &delta_param,const mat &gama,const mat &beta,const vec &penal_param,const vec &stop_param,
                        const vec &b_l,const vec &b_r,
                        const vec &nu_l,const vec &nu_r,
                        const vec &nu_l_p,const vec &nu_r_p,
                        const vec &nu_l_s,const vec &nu_r_s,
                        ...
                        vec &weight_FS_LCR, vec &weight_FS_LIR,vec &weight_FS_RCR, vec &weight_FS_RIR,
                        ...
                        const vec &lower_bound_init,
                        
                        vec &lower_bound,
                        
                        const double upper_bound,const bool lt = 1){
  ...
  // Rcpp::Rcout<<"ABG"<< endl;
    ...
    vec p(size(delta_param),fill::randn);


    double U_old=-log_lhood(Inhibit_likelihood,
                            sigma, tau_prime, tau_s, SSD,Ind_GF,
                            b_l, b_r, nu_l, nu_r, nu_l_p, nu_r_p, nu_l_s, nu_r_s,
                            Ind_L, Ind_R, Ind_LCR, Ind_LIR, Ind_RIR, Ind_RCR,
                            Ind_S_LCR, Ind_S_LIR, Ind_S_RIR, Ind_S_RCR,
                            lk_LCR_FS, lk_LIR_FS,lk_RCR_FS,lk_RIR_FS,
    ...
    double prior_main=delta_param(1)-(2*log1p(exp(delta_param(1))))+
                      delta_param(0)-(2*log1p(exp(delta_param(0))));
    ...
    vec v_old =-    grad_delta(tau_prime,tau_s,sigma,SSD,DEL,DEL_s,
                               delta_param,stop_param,
                               nu_l, nu_r,b_l, b_r,
                               nu_l_p,nu_r_p,nu_l_s,nu_r_s,delta_prime_Stim,delta_s_Stim,
                               weight_LIR, weight_RCR,weight_LCR,weight_RIR,
                               weight_LCR_G_FS, weight_LIR_G_FS,weight_RCR_G_FS,weight_RIR_G_FS,
    ...
    vec delta_param_new=delta_param;
    ...
    leap_frog_delta_param(tau,tau_stop,sigma,SSD,SSD_min,U,
                          stop_param,penal_param,prob_param,delta_param_new,
                          b_l,b_r,
                          nu_l,nu_r,
                          nu_l_p,nu_r_p,nu_l_s,nu_r_s,
                          Ind_L,Ind_R,
                          Ind_LCR,Ind_LIR,Ind_RIR,Ind_RCR,
                          Ind_S_LCR,Ind_S_LIR,Ind_S_RIR,Ind_S_RCR,
                          Ind_I_L,Ind_I_R,
                          v_new,nstep,delta,p,lower_bound_init,upper_bound,lt);




    double DEL_s_new=U*EXPITE(delta_param_new(0));
    ...
    vec tau_prime_new(tau.n_elem, fill::zeros);
    vec tau_s_new(tau_stop.n_elem, fill::zeros);

    tau_prime_new.elem(idx_tau_pos) = tau.elem(idx_tau_pos) - DEL_new;

    tau_s_new.elem(idx_tau_stop_pos) = tau_stop.elem(idx_tau_stop_pos) - DEL_s_new;


    vec lower_bound_new = lower_bound_init+DEL_s_new;



    field<vec> likelihood_integral_new=integral_likelihood2(sigma,SSD,DEL_new,DEL_s_new,
                                                           stop_param,penal_param,prob_param,
                                                           b_l,nu_l,nu_l_s,
                                                           b_r,nu_r_p,nu_r_s,
                                                           nu_r,nu_l_p,
    ...
    field<vec> lk_FS_new= update_lk_FS4(tau_prime_new,tau_s_new,sigma,SSD,
                                        b_l,b_r,nu_l,nu_r,nu_r_p,nu_l_p,
                                        nu_l_s,nu_r_s,
                                        Ind_L,Ind_R,Ind_LCR,Ind_LIR,Ind_RCR,Ind_RIR,
                                        Ind_S_LCR,Ind_S_LIR,Ind_S_RCR,Ind_S_RIR,
                                        stop_param,prob_param,DEL_new,DEL_s_new,lt);
    ...
    double U_new=-log_lhood(Inhib_likelihood_new,
                            sigma, tau_prime_new, tau_s_new, SSD,Ind_GF,
                            b_l, b_r, nu_l, nu_r, nu_l_p, nu_r_p, nu_l_s, nu_r_s,
                            Ind_L, Ind_R, Ind_LCR, Ind_LIR, Ind_RIR, Ind_RCR,
                            Ind_S_LCR, Ind_S_LIR, Ind_S_RIR, Ind_S_RCR,
                            lk_LCR_FS_new, lk_LIR_FS_new,lk_RCR_FS_new,lk_RIR_FS_new,
    ...
    double prior_main_new=delta_param_new(1)-(2*log1p(exp(delta_param_new(1))))+
                          delta_param_new(0)-(2*log1p(exp(delta_param_new(0))));
    ...
    accpt_del_samp=log(randu()) < -(H_new - H_old);

    if (accpt_del_samp) {

        delta_param = delta_param_new;
        
        DEL = DEL_new;
        DEL_s =DEL_s_new;
      
        tau_prime = tau_prime_new;
        tau_s = tau_s_new;
        ...
        weight_LCR=weight(sigma,g(tau_prime, Ind_L, Ind_LCR ),h(nu_r_p,Ind_LCR ),g(b_r, Ind_L, Ind_LCR ));
        
        weight_LIR=weight(sigma,g(tau_prime, Ind_L, Ind_LIR ),g(nu_l, Ind_L, Ind_LIR ),g(b_l, Ind_L, Ind_LIR ));
        
        weight_RCR=weight(sigma,g(tau_prime, Ind_R, Ind_RCR ),h(nu_l_p, Ind_RCR ),g(b_l, Ind_R, Ind_RCR));
        
        weight_RIR=weight(sigma,g(tau_prime, Ind_R, Ind_RIR ),g(nu_r, Ind_R, Ind_RIR ),g(b_r, Ind_R, Ind_RIR ));
        
        
        weight_LCR_G_FS=weight(sigma,g(tau_prime, Ind_L, Ind_S_LCR ),h(nu_r_p, Ind_S_LCR ),g(b_r, Ind_L, Ind_S_LCR ));
        
        weight_LIR_G_FS=weight(sigma,g(tau_prime, Ind_L, Ind_S_LIR ),g(nu_l, Ind_L, Ind_S_LIR ),g(b_l, Ind_L, Ind_S_LIR ));
        
        weight_RCR_G_FS=weight(sigma,g(tau_prime, Ind_R, Ind_S_RCR ),h(nu_l_p, Ind_S_RCR ),g(b_l, Ind_R, Ind_S_RCR ));
        
        weight_RIR_G_FS=weight(sigma,g(tau_prime, Ind_R, Ind_S_RIR ),g(nu_r, Ind_R, Ind_S_RIR ),g(b_r, Ind_R, Ind_S_RIR ));
        
        
        weight_LCR_S=weight_s(sigma,g(tau_prime, Ind_L, Ind_S_LCR ),g(tau_s, Ind_L, Ind_S_LCR ),g(SSD, Ind_L, Ind_S_LCR ),
                              g(b_r, Ind_L, Ind_S_LCR ),h(nu_r_p, Ind_S_LCR ),
                              g(nu_r_s, Ind_L, Ind_S_LCR ),DEL,DEL_s,lt);
        
        weight_LIR_S=weight_s(sigma,g(tau_prime, Ind_L, Ind_S_LIR ),g(tau_s, Ind_L, Ind_S_LIR ),g(SSD, Ind_L, Ind_S_LIR ),
                              g(b_l, Ind_L, Ind_S_LIR ),g(nu_l,Ind_L, Ind_S_LIR ),
                              g(nu_l_s, Ind_L, Ind_S_LIR ),DEL,DEL_s,lt);
        
        weight_RCR_S=weight_s(sigma,g(tau_prime, Ind_R, Ind_S_RCR ),g(tau_s, Ind_R, Ind_S_RCR ),g(SSD, Ind_R, Ind_S_RCR ),
                              g(b_l, Ind_R, Ind_S_RCR ),h(nu_l_p, Ind_S_RCR ),
                              g(nu_l_s,Ind_R, Ind_S_RCR ),DEL,DEL_s,lt);
        
        weight_RIR_S=weight_s(sigma, g(tau_prime, Ind_R, Ind_S_RIR ),g(tau_s, Ind_R, Ind_S_RIR ),g(SSD, Ind_R, Ind_S_RIR ),
                              g(b_r, Ind_R, Ind_S_RIR ),g(nu_r, Ind_R, Ind_S_RIR ),
                              g(nu_r_s,Ind_R, Ind_S_RIR ),DEL,DEL_s,lt);
        
        
        weight_FS_LCR=weight_stop(g(tau_s, Ind_L, Ind_S_LCR ), sigma,stop_param);
        
        weight_FS_LIR=weight_stop(g(tau_s, Ind_L, Ind_S_LIR ), sigma,stop_param);
        
        weight_FS_RCR=weight_stop(g(tau_s, Ind_R, Ind_S_RCR ), sigma,stop_param);
        
        weight_FS_RIR=weight_stop(g(tau_s, Ind_R, Ind_S_RIR ), sigma,stop_param);
...
void leap_frog_stop_prob_param(const vec &tau, const vec &tau_s, const double sigma,const vec &delta_prime,
                               ...
                               vec &v_old_stop,vec &v_old_prob,const unsigned L, const double delta,
                               vec &p_stop,vec &p_prob,
...
void update_stop_prob_param(const vec &tau, const vec &tau_s, const double sigma,const vec &SSD,
                            const double DEL,const double DEL_s,
                            const vec &Ind_GF,const vec &delta_prime,
                            const mat &gama, const mat &beta,const vec &penal_param, vec &stop_param,vec &prob_param,
                            const vec &b_l, const vec &b_r,
                            const vec &nu_l, const vec &nu_r,
                            const vec &nu_l_p, const vec &nu_r_p,
                            
                            vec &nu_l_s,vec &nu_r_s,
                            const vec &nu_l_squared, const vec &nu_r_squared,
                            ...
                            const vec &range, const unsigned L,
                            const int leapmax,  unsigned nparall,
                            bool &accpt_stop_prob_samp,
                            const vec &lower_bound, const double upper_bound, const bool lt = true) {
  ...
  // Rcpp::Rcout<<"ABK"<< endl;
    ...
    vec p_stop(size(stop_param),fill::randn);
    
    vec p_prob(size(prob_param),fill::randn);
    ...
    double U_old=-(log_lhood(Inhib_likelihood,
                             ...
                             Ind_S_LCR, Ind_S_LIR, Ind_S_RIR, Ind_S_RCR,
                             lk_LCR_FS, lk_LIR_FS,lk_RCR_FS,lk_RIR_FS,
                             prob_param,T_Go,k)-(T_Total*sum_Prob));
    
    
    
    double prior_main_prob=log_gama_prior(prob_param,prob_hyp(0),prob_hyp(1),prob_hyp(2));
    
    
    double prior_main_stop=log_normal_prior(stop_param,mu_b_stop,sigma_b_stop,mu_nu_stop,sigma_nu_stop);
    ...
    vec v_old_stop = -grad_stop_param(tau,tau_s,sigma,SSD,DEL,DEL_s,
                                      nu_l,nu_r,b_l,b_r,
                                      nu_l_p,nu_r_p,nu_l_s,nu_r_s,
                                      weight_LCR_S, weight_LIR_S,weight_RCR_S,weight_RIR_S,
                                      weight_FS_LCR,weight_FS_LIR,weight_FS_RCR,weight_FS_RIR,
    ...
    vec v_old_prob = -grad_prob_param(tau_s,sigma,likelihood_integral,
                                      stop_param,prob_param,sum_Prob,
                                      prob_hyp,
                                      T_Go,k,T_Total,
                                      diff_X_LCR,diff_Y_LCR,diff_X_LIR,diff_Y_LIR,
                                      diff_X_RCR,diff_Y_RCR,diff_X_RIR,diff_Y_RIR);
    ...
    vec stop_param_new=stop_param;
    
    vec v_new_stop = v_old_stop;
    
    
    vec prob_param_new=prob_param;
    ...
    leap_frog_stop_prob_param(tau, tau_s, sigma,delta_prime,SSD,DEL,DEL_s,
                              b_l, b_r,
                              nu_l, nu_r, nu_l_p, nu_r_p,
                              nu_l_squared, nu_r_squared,
                              ...
                              Ind_S_LCR, Ind_S_LIR, Ind_S_RCR, Ind_S_RIR,
                              Ind_I_L, Ind_I_R,
                              stop_param_new, penal_param,prob_param_new,
    ...
    vec nu_l_s_i_new = nu_l - penalty_fun_s(stop_param_new(1), penal_param);
    vec nu_r_s_i_new = nu_r - penalty_fun_s(stop_param_new(1), penal_param);
    
    
    field<vec> likelihood_integral_new = integral_likelihood2(sigma,SSD,DEL,DEL_s,stop_param_new,
                                                              penal_param,prob_param_new,
                                                              b_l,nu_l,nu_l_s_i_new,
                                                              b_r,nu_r_p,nu_r_s_i_new,
                                                              nu_r,nu_l_p,
                                                              Ind_L,Ind_R,Ind_I_L,Ind_I_R,
                                                              lower_bound,upper_bound,lt);
    ...
    field<vec> lk_FS_new= update_lk_FS4(tau,tau_s,sigma,SSD,
                                        ...
                                        Ind_L,Ind_R,Ind_LCR,Ind_LIR,Ind_RCR,Ind_RIR,
                                        Ind_S_LCR,Ind_S_LIR,Ind_S_RCR,Ind_S_RIR,
                                        stop_param_new,prob_param_new,DEL,DEL_s,lt);
    ...
    double U_new=- (log_lhood(Inhib_likelihood_new,
                              sigma, tau, tau_s, SSD,Ind_GF,
                              b_l, b_r, nu_l, nu_r, nu_l_p, nu_r_p, nu_l_s_i_new, nu_r_s_i_new,
                              Ind_L, Ind_R, Ind_LCR, Ind_LIR, Ind_RIR, Ind_RCR,
                              Ind_S_LCR, Ind_S_LIR, Ind_S_RIR, Ind_S_RCR,
                              lk_LCR_FS_new, lk_LIR_FS_new,lk_RCR_FS_new,lk_RIR_FS_new,
                              prob_param_new,T_Go,k)-(T_Total*sum_Prob_new));
    ...
    accpt_stop_prob_samp = (log(randu()) < -(H_new - H_old));
    
    
    if (accpt_stop_prob_samp) {
  
        stop_param = stop_param_new;
        prob_param = prob_param_new;
...
List hmc(const List Time_stamp,const List Go_RT,const List Go_RT_S,const double SSD_min,const arma::vec& U,const List Ind_G,const List Stop_S_D,
  ...
  field <mat> gama_Indiv=make_field_from_list1(gama_Ind);
  field <mat> beta_Indiv=make_field_from_list1(beta_Ind);
  ...
  // vec acceptance_stop(N, fill::zeros);
  //
  // vec acceptance_prob(N, fill::zeros);
  ...
  field<field<vec>> gama_ess2=update_gama_param_ess3(b_l_main,b_r_main,Phi,gama_Indiv);
  ...
  for (unsigned i = 0; i < nhmc; ++i){
    ...
    // Updating all individual parameter
...
#pragma omp parallel for num_threads(nparall)
      for (unsigned s = 0; s < N; ++s) {
        ...
        // Rcpp::Rcout<<"sub: "<< s <<endl;
        
        
        bool accpt_del_samp       = false;
        bool accpt_rand_samp      = false;
        bool accpt_p_samp         = false;
        bool accpt_stop_prob_samp = false;
        
        vec delta_param_i = delta_param.col(s);
        vec penal_param_i = penal_param.col(s);
        vec stop_param_i  = stop_param.col(s);
        vec prob_param_i  = prob_param.col(s);
       
        
        mat gama_I = gama_Indiv(s);
        mat beta_I = beta_Indiv(s);
        
        
        double DEL_s_i=U(s)*EXPITE(delta_param_i(0));
        
        double DEL_i=(SSD_min+DEL_s_i)*EXPITE(delta_param_i(1));
        
         
        vec tau_i = tau(s);
        vec tau_stop_i = tau_stop(s);
        ...
        tau_prime.elem(idx_tau_pos) = tau_i.elem(idx_tau_pos) - DEL_i;
        
        uvec idx_tau_stop_pos = find(tau_stop_i > 0);
        tau_s.elem(idx_tau_stop_pos) = tau_stop_i.elem(idx_tau_stop_pos) - DEL_s_i;
        
        vec lower_bound=lower_bound_init(s)+DEL_s_i;
        
        
        
        
        vec b_l = b_l_main(s)+ (Phi(s) * gama_I.col(0));
        vec b_r = b_r_main(s)+ (Phi(s) * gama_I.col(1));
        
        
        
        vec nu_l = nu_l_main(s)+ (Phi(s) * beta_I.col(0));
        vec nu_l_squared = square(nu_l);
        
        
        vec nu_r = nu_r_main(s)+ (Phi(s) * beta_I.col(1));
        vec nu_r_squared = square(nu_r);
        
        
        vec nu_l_p = h(nu_l, Ind_R(s)) - penalty_fun(h(nu_r, Ind_R(s) ),h(nu_r_squared, Ind_R(s) ),penal_param_i);
        vec nu_r_p = h(nu_r, Ind_L(s)) - penalty_fun(h(nu_l, Ind_L(s) ),h(nu_l_squared, Ind_L(s) ),penal_param_i);
        
        
        vec nu_l_s=nu_l-penalty_fun_s(stop_param_i(1),penal_param_i);
        vec nu_r_s=nu_r-penalty_fun_s(stop_param_i(1),penal_param_i);
        
       
        
        
        vec weight_LCR=weight(sigma,g(tau_prime, Ind_L(s), Ind_LCR(s) ),h(nu_r_p, Ind_LCR(s) ),
                              g(b_r, Ind_L(s), Ind_LCR(s) ));
        
        vec weight_LIR=weight(sigma,g(tau_prime, Ind_L(s), Ind_LIR(s) ),g(nu_l, Ind_L(s),Ind_LIR(s) ),
                              g(b_l, Ind_L(s), Ind_LIR(s) ));
        
        vec weight_RIR=weight(sigma,g(tau_prime, Ind_R(s), Ind_RIR(s) ),g(nu_r, Ind_R(s), Ind_RIR(s) ),
                              g(b_r, Ind_R(s), Ind_RIR(s) ));
        
        vec weight_RCR=weight(sigma,g(tau_prime, Ind_R(s), Ind_RCR(s) ),h(nu_l_p, Ind_RCR(s) ),
                              g(b_l, Ind_R(s), Ind_RCR(s) ));
        
        
        
        
        vec weight_LCR_G_FS=weight(sigma,g(tau_prime, Ind_L(s), Ind_S_LCR(s) ),h(nu_r_p, Ind_S_LCR(s) ),g(b_r, Ind_L(s), Ind_S_LCR(s) ));
        
        vec weight_LIR_G_FS=weight(sigma,g(tau_prime, Ind_L(s), Ind_S_LIR(s) ),g(nu_l, Ind_L(s), Ind_S_LIR(s) ),
                                   g(b_l, Ind_L(s), Ind_S_LIR(s) ));
        
        vec weight_RCR_G_FS=weight(sigma,g(tau_prime, Ind_R(s), Ind_S_RCR(s) ),h(nu_l_p, Ind_S_RCR(s) ),g(b_l, Ind_R(s), Ind_S_RCR(s) ));
        
        vec weight_RIR_G_FS=weight(sigma,g(tau_prime, Ind_R(s), Ind_S_RIR(s) ),g(nu_r, Ind_R(s), Ind_S_RIR(s) ),
                                   g(b_r, Ind_R(s), Ind_S_RIR(s) ));
        
        
        vec weight_LCR_S=weight_s(sigma,g(tau_prime, Ind_L(s), Ind_S_LCR(s) ),g(tau_s, Ind_L(s), Ind_S_LCR(s) ),g(SSD(s), Ind_L(s), Ind_S_LCR(s) ),
                                  g(b_r, Ind_L(s), Ind_S_LCR(s) ),h(nu_r_p, Ind_S_LCR(s) ),
                                  g(nu_r_s, Ind_L(s), Ind_S_LCR(s) ),DEL_i,DEL_s_i,lt);
        
        vec weight_LIR_S=weight_s(sigma,g(tau_prime, Ind_L(s), Ind_S_LIR(s) ),g(tau_s, Ind_L(s), Ind_S_LIR(s) ),g(SSD(s), Ind_L(s), Ind_S_LIR(s) ),
                                  g(b_l, Ind_L(s), Ind_S_LIR(s) ),g(nu_l,Ind_L(s), Ind_S_LIR(s) ),
                                  g(nu_l_s, Ind_L(s), Ind_S_LIR(s) ),DEL_i,DEL_s_i,lt);
        
        vec weight_RCR_S=weight_s(sigma,g(tau_prime, Ind_R(s), Ind_S_RCR(s) ),g(tau_s, Ind_R(s), Ind_S_RCR(s) ),g(SSD(s), Ind_R(s), Ind_S_RCR(s) ),
                                  g(b_l, Ind_R(s), Ind_S_RCR(s) ),h(nu_l_p, Ind_S_RCR(s) ),
                                  g(nu_l_s,Ind_R(s), Ind_S_RCR(s) ),DEL_i,DEL_s_i,lt);
        
        
        vec weight_RIR_S=weight_s(sigma, g(tau_prime, Ind_R(s), Ind_S_RIR(s) ),g(tau_s, Ind_R(s), Ind_S_RIR(s) ),g(SSD(s), Ind_R(s), Ind_S_RIR(s) ),
                                  g(b_r, Ind_R(s), Ind_S_RIR(s) ),g(nu_r, Ind_R(s), Ind_S_RIR(s) ),
                                  g(nu_r_s,Ind_R(s), Ind_S_RIR(s) ),DEL_i,DEL_s_i,lt);
        
        
        
        vec weight_FS_LCR=weight_stop(g(tau_s, Ind_L(s), Ind_S_LCR(s)), sigma,stop_param_i);
        vec weight_FS_LIR=weight_stop(g(tau_s, Ind_L(s), Ind_S_LIR(s)), sigma,stop_param_i);
        vec weight_FS_RCR=weight_stop(g(tau_s, Ind_R(s), Ind_S_RCR(s)), sigma,stop_param_i);
        vec weight_FS_RIR=weight_stop(g(tau_s, Ind_R(s), Ind_S_RIR(s)), sigma,stop_param_i);
        
        
        
        
        field<vec> likelihood_integral=integral_likelihood2(sigma,SSD(s),DEL_i,DEL_s_i,
                                                           stop_param_i,penal_param_i,prob_param_i,
                                                           b_l,nu_l,nu_l_s,
                                                           b_r,nu_r_p,nu_r_s,
                                                           nu_r,nu_l_p,
                                                           Ind_L(s),Ind_R(s),Ind_I_L(s),Ind_I_R(s),
                                                           lower_bound,upper_bound,lt);
        
        
        
        
        field<vec> lk_FS=update_lk_FS4(tau_prime,tau_s, sigma,SSD(s),
                                       b_l,b_r,nu_l,nu_r,
                                       nu_r_p,nu_l_p,nu_l_s,nu_r_s,
                                       Ind_L(s),Ind_R(s),Ind_LCR(s), Ind_LIR(s),Ind_RCR(s),Ind_RIR(s),
                                       Ind_S_LCR(s),Ind_S_LIR(s),Ind_S_RCR(s),Ind_S_RIR(s),
                                       stop_param_i,prob_param_i,DEL_i,DEL_s_i,lt);
        
        
        double sum_Prob = mlpack::AccuLog(prob_param_i);
        
        
        
        
        update_delta_param(tau_i,tau_stop_i,tau_prime,tau_s,sigma,SSD(s),SSD_min,U(s),
                           DEL_i,DEL_s_i,Ind_GF(s),
                           delta_param_i,gama,beta,penal_param_i,stop_param_i,
                           b_l,b_r,nu_l,nu_r,nu_l_p,nu_r_p,nu_l_s,nu_r_s,
                           Ind_L(s),Ind_R(s),
                           Ind_LCR(s),Ind_LIR(s),Ind_RCR(s),Ind_RIR(s),
                           Ind_S_LCR(s),Ind_S_LIR(s),Ind_S_RCR(s),Ind_S_RIR(s),
                           Ind_I_L(s),Ind_I_R(s),
                           
                           prob_param_i,T_Go(s),k(s),
                           ...
                           lk_FS,likelihood_integral,
                           
                           range_d,L,leapmax,
                           nparall,accpt_del_samp,
                           
                           lower_bound_init(s),lower_bound,
                           upper_bound,lt);
        
        
        if (accpt_del_samp) {
          delta_param.col(s) = delta_param_i;
        ...
        update_rand_effect(Phi(s),
                           b_l_main(s),b_r_main(s),nu_l_main(s),nu_r_main(s),
                           S_gama_l,S_gama_r,S_beta_l,S_beta_r,
                           rand_param_g_l,rand_param_g_r,rand_param_b_l,rand_param_b_r,
                           tau_prime,tau_s,sigma,SSD(s),DEL_i,DEL_s_i,
                           Ind_GF(s),delta_param_i,penal_param_i,stop_param_i,prob_param_i,
                           ...
                           b_l,nu_l,b_r,nu_r,nu_l_p,nu_r_p,nu_l_s,nu_r_s,
                           nu_l_squared,nu_r_squared,
                           ...
                           range_rand_effect,L,leapmax, nparall,
                           lower_bound,upper_bound,
                           accpt_rand_samp,lt);
        ...
        if (accpt_rand_samp) {
          ...
          acceptance_rand_effect[s]++;
        }
        
        
        update_penal_param(tau_prime,tau_s,sigma,SSD(s),DEL_i,DEL_s_i,Ind_GF(s),
                           delta_param_i,gama,beta,penal_param_i,stop_param_i,
                           b_l,b_r,nu_l,nu_r,nu_l_squared,nu_r_squared,
                           ...
                           lk_FS,likelihood_integral,prob_param_i,
                           T_Go(s),k(s),
                           
                           range_p,L,leapmax,nparall,
                           
                           accpt_p_samp,lower_bound,upper_bound,lt);
      
        if (accpt_p_samp) {
         
          penal_param.col(s)=penal_param_i;
          acceptance_p[s]++;
        }
        
        
        update_stop_prob_param(tau_prime,tau_s,sigma,SSD(s),DEL_i,DEL_s_i,
                               Ind_GF(s),delta_param_i,gama,beta,penal_param_i,stop_param_i,prob_param_i,
                               b_l, b_r,nu_l, nu_r,nu_l_p,nu_r_p,
                               
                               nu_l_s,nu_r_s,nu_l_squared,nu_r_squared,
                               ...
                               prob_hyp,T_Go(s),k(s),T_Total(s),sum_Prob,
                               ...
                               range_stop_prob,L,leapmax,nparall,accpt_stop_prob_samp,
                               lower_bound,upper_bound,lt);
        ...
        if (accpt_stop_prob_samp) {
          ...
          stop_param.col(s) = stop_param_i;
          prob_param.col(s) = prob_param_i;
          acceptance_stop_prob[s]++;
        }
      ...
      field<field<vec>>  beta_ess4=update_beta_param_ess4(nu_l_main,nu_r_main,Phi,beta_Indiv);
    ...
    if(update_rand_eff==1){
      ...
      field<mat> sq_ess= update_sq(gama_Indiv);
      
      mat gama_sq_l=sq_ess(0);
      mat gama_sq_r=sq_ess(1);
      
      
      field<mat> sq_ess2= update_sq(beta_Indiv);
    ...
    if (i % thin == 0) {
      ...
      for (unsigned j = 0; j < N; ++j) {
        theta5_L.slice(i/thin).col(j) = gama_Indiv(j).col(0);
        theta5_R.slice(i/thin).col(j) = gama_Indiv(j).col(1);
        theta6_L.slice(i/thin).col(j) = beta_Indiv(j).col(0);
        theta6_R.slice(i/thin).col(j) = beta_Indiv(j).col(1);
      }
  ...
  // out["Accept_Stop"]=acceptance_stop/nhmc;
  // out["Accept_Prob"]=acceptance_prob/nhmc;
